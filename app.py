...
@app.route('/chapter')
def chapter():
    chapter_number = int (request.args.get('chapter_number'))
    return render_template('chapter.html', chapter_number=chapter_number)



@app.route('/chapters')
def chapters():
    return render_template('chapters.html', chapters=range(1, 11))
...
def generate_word_chapter_2():
    ...
    word_path = os.path.join("static", "generated", "Chapter_2_Assignment.docx")
    doc = Document()
    ...
    host = request.root_url #urlparse(request.base_url)
    doc.add_paragraph(f"📂 Duplicated Dataset: {host}/{duplicated_path}")
    doc.add_paragraph("\n✅ Answer Key:")
    doc.add_paragraph(f"📂 Original Dataset: {host}/{original_path}")

    print(word_path, request.path)
